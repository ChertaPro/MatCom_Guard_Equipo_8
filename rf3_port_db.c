...
{
    if (puerto <= 1024)
        return "[ALERTA] Puerto privilegiado no registrado";
    else if (puerto > 1024 && puerto <= 49151)
        return "[INFO] Puerto registrado no catalogado";
    else
        return "[OBSERVADO] Puerto efímero inusual";
}
